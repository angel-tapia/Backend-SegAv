...
`;
    ...
    generateDelegateRelationForBase(model, decl) {
        ...
        concreteModels.forEach((concrete) => {
            const auxName = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${(0, lower_case_first_1.lowerCaseFirst)(concrete.name)}`;
    ...
    generateDelegateRelationForConcrete(model, concreteDecl) {
        ...
        baseModels.forEach((base) => {
            ...
            // add relation fields
            const relationField = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${(0, lower_case_first_1.lowerCaseFirst)(base.name)}`;
    ...
    expandPolymorphicRelations(model, decl) {
        ...
        decl.fields.forEach((f) => {
            var _a;
            const fieldType = (_a = f.type.reference) === null || _a === void 0 ? void 0 : _a.ref;
            if (!(0, ast_1.isDataModel)(fieldType)) {
                return;
            }
            // find concrete models that inherit from this field's model type
            const concreteModels = decl.$container.declarations.filter((d) => (0, ast_1.isDataModel)(d) && isDescendantOf(d, fieldType));
            // aux relation name format: delegate_aux_[model]_[relationField]_[concrete]
            // e.g., delegate_aux_User_myAsset_Video
            concreteModels.forEach((concrete) => {
                const relationField = model.addField(`${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${decl.name}_${f.name}_${concrete.name}`, new prisma_builder_1.ModelFieldType(concrete.name, f.type.array, f.type.optional));
                const relAttr = (0, sdk_1.getAttribute)(f, '@relation');
                if (relAttr) {
                    const fieldsArg = relAttr.args.find((arg) => arg.name === 'fields');
                    if (fieldsArg) {
                        const idFields = (0, ast_utils_1.getIdFields)(fieldType);
                        // add fk fields, e.g., delegate_aux_User_myAsset_VideoId
                        const addedIdFields = idFields.map((idField) => model.addField(`${relationField.name}${(0, upper_case_first_1.upperCaseFirst)(idField.name)}`, idField.type.type));
                        const fieldsArg = new prisma_builder_1.AttributeArgValue('Array', addedIdFields.map((f) => new prisma_builder_1.AttributeArgValue('FieldReference', new prisma_builder_1.FieldReference(f.name))));
                        const referencesArg = new prisma_builder_1.AttributeArgValue('Array', idFields.map((f) => new prisma_builder_1.AttributeArgValue('FieldReference', new prisma_builder_1.FieldReference(f.name))));
                        const addedRel = new prisma_builder_1.FieldAttribute('@relation', [
                            // use field name as relation name for disambiguation
                            new prisma_builder_1.AttributeArg(undefined, new prisma_builder_1.AttributeArgValue('String', relationField.name)),
                        ...
                        if (this.supportNamedConstraints) {
                            ...
                            new prisma_builder_1.AttributeArg('map', new prisma_builder_1.AttributeArgValue('String', `${relationField.name}_fk`)));
                        }
                        relationField.attributes.push(addedRel);
                    }
                    else {
                        relationField.attributes.push(this.makeFieldAttribute(relAttr));
                    }
                }
                else {
                    relationField.attributes.push(new prisma_builder_1.FieldAttribute('@relation', [
                        // use field name as relation name for disambiguation
                        new prisma_builder_1.AttributeArg(undefined, new prisma_builder_1.AttributeArgValue('String', relationField.name)),
                    ]));
                }
    ...
    nameRelationsInheritedFromDelegate(model, decl) {
        ...
        decl.fields.forEach((f) => {
            var _a;
            if (!f.$inheritedFrom || !(0, sdk_1.isDelegateModel)(f.$inheritedFrom) || !(0, ast_1.isDataModel)((_a = f.type.reference) === null || _a === void 0 ? void 0 : _a.ref)) {
                return;
            }
            ...
            const relAttr = (0, sdk_1.getAttribute)(f, '@relation');
            const relName = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${fieldType.name}_${oppositeRelationField.name}_${decl.name}`;
            if (relAttr) {
